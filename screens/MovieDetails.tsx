...
export default function MovieDetails() {
  ...
  useEffect(
    function loadMovieData() {
      ...
      API.get(`/movie/${route.params.movieId}?language=pt-BR'`).then(
        (response) => setMovie(response.data)
      );
      API.get(`/movie/${route.params.movieId}/credits?language=pt-BR'`).then(
    ...
    <Container>
      <ScrollView>
          ...
          <FlatList
            ...
            renderItem={({ item: genre }) => (
              <View className="p-1 bg-[#F02D4C] rounded mx-2">
                <Text className="text-white text-md">{genre.name}</Text>
              </View>
            )}
            horizontal
        ...
        <View className="px-2 my-2">
          <Text className="text-white text-md">{movie.overview}</Text>
        </View>
        <View>
          <FlatList
            data={cast}
            renderItem={({ item: member }) => <CastCard castMember={member} />}
            horizontal
